...
MO_LOCALE_DIRNAME = "locale"
MO_CATEGORY_DIRNAME = "LC_MESSAGES"


class MOPathError(OSError) :
    pass
...
def gen_msg_catalog_template(po_path) :
    ...
    xgettext_cmd_str = ("xgettext"
                        " --keyword=TR_KV:2"
                        " --keyword=TR_KV_FMT:2"
                        " --keyword=TR_PROTECTED_KV:2"
                        " --language=C++ --sort-by-file --output="
                        + po_dir_path
                        + '/'
                        + DOMAIN_NAME
                        + ".pot "
    ...
    if subprocess.call(xgettext_cmd_str,
                       shell=True,
                       stdout=subprocess.DEVNULL,
                       stderr=subprocess.STDOUT) != 0 :
        raise OSError("error during message catalog template generation")
...
def gen_transl_msg_catalog(target_locale, po_path) :
    ...
    locale_cmd_str = "locale -a"
    grep_cmd_str = "grep \"" + target_locale + "_*\""

    if subprocess.call(locale_cmd_str + '|' + grep_cmd_str,
                       shell=True,
                       stdout=subprocess.DEVNULL,
                       stderr=subprocess.STDOUT) != 0 :
        raise OSError("locale '" + target_locale + "' doesn't exist in system")
    ...
    msginit_cmd_str = ("msginit --no-translator --no-wrap --locale="
                       + target_locale
                       + " --input="
                       + po_dir_path
                       + '/'
    ...
    if subprocess.call(msginit_cmd_str,
                       shell=True,
                       stdout=subprocess.DEVNULL,
                       stderr=subprocess.STDOUT) != 0 :
        raise OSError("error during translated message catalog generation")
...
def gen_modified_transl_msg_catalog_from_tpl(target_locale, po_path) :
    ...
    msgmerge_cmd_str = ("msgmerge --no-wrap --update "
                        + target_locale_dir_path
                        + '/'
                        + DOMAIN_NAME
                        + ".po "
    ...
    if subprocess.call(msgmerge_cmd_str,
                       shell=True,
                       stdout=subprocess.DEVNULL,
                       stderr=subprocess.STDOUT) != 0 :
        raise OSError("error during translated message catalog modification "
                      "from message catalog template")
...
def gen_compiled_msg_catalog(target_locale, po_path, mo_path) :
    ...
    if not os.path.exists(mo_path) :
        raise MOPathError("missing '" + mo_path + "' directory")
    
    mo_locale_dir_path = mo_path + '/' + MO_LOCALE_DIRNAME

    if not os.path.exists(mo_locale_dir_path) :
        os.mkdir(mo_locale_dir_path)
        print("'" + mo_locale_dir_path + "' directory has been generated")
    elif not os.path.isdir(mo_locale_dir_path) :
        raise OSError("cannot use '" + mo_locale_dir_path + "' as directory")

    mo_target_locale_dir_path = mo_locale_dir_path + '/' + target_locale
    ...
    msgfmt_cmd_str = ("msgfmt --output-file="
                      + mo_category_dir_path
                      + '/'
                      + DOMAIN_NAME
                      + ".mo "
                      + po_target_locale_dir_path
                      + '/'
                      + DOMAIN_NAME
                      + ".po")
    
    if subprocess.call(msgfmt_cmd_str,
                       shell=True,
                       stdout=subprocess.DEVNULL,
                       stderr=subprocess.STDOUT) != 0 :
