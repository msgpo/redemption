...
void config_spec_definition(Writer && W)
{
    ...
    W.section("globals", [&]
    {
        W.member(no_ini_no_gui, proxy_to_sesman, not_target_ctx, L,
            type_<std::chrono::milliseconds>(), "front_connection_time",
            desc{"from incoming connection to \"up_and_running\" state"});
        W.member(no_ini_no_gui, proxy_to_sesman, not_target_ctx, L,
            type_<std::chrono::milliseconds>(), "target_connection_time",
        ...
        W.member(ini_and_gui, no_sesman, L, type_<std::chrono::seconds>(), "session_timeout", desc{"No traffic auto disconnection, timer is set after authentication on primary session.\nIf value is smaller than 30, 30 is used by default (except 0 which unlimited value is used)."}, set(900));
        W.member(hidden_in_gui, rdp_connpolicy, connpolicy::section{"session"}, L, type_<std::chrono::seconds>(), "inactivity_timeout", desc{"No traffic auto disconnection, timer is set on secondary session.\nIf value is smaller than 30, 30 is used by default (except 0 which global value \"session_timeout\" is used instead)."}, set(0));
    ...
    W.section("theme", [&]
    {
        ...
        W.member(image_in_gui, no_sesman, L, type_<std::string>(), "logo", desc{"File path for upload logo"}, set(CPP_EXPR(REDEMPTION_CONFIG_THEME_LOGO)));
